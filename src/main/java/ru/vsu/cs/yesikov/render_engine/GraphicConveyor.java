...
/*
    ...
    public static Vector3f multiplyMatrix4ByVector3(final Matrix4x4 matrix, final Vector3f vertex) {
        Vector4f v = new Vector4f(vertex.getX(), vertex.getY(), vertex.getZ(), 1);
    ...
    public static Point2f vertexToPoint(Vector3f vertex, int width, int height) {
        return new Point2f(vertex.getX() * (float)width + (float)width / 2.0F, -vertex.getY() * (float)height + (float)height / 2.0F);
    }
}
